...
struct wasp_session_s {         // WASP session
   wasp_session_t **prev,
   *next;                       // Sessions
...
struct wasp_chanlink_s {        // Link a channel to a session
...
static char *tokens(char *s, unsigned char n)
...
static void link_channel(wasp_session_t * s, wasp_channel_t * c)
{
   if (debug)
...
static void unlink_channel(wasp_chanlink_t * l)
{                               // Already under mutex
   if (debug)
...
static char *wasp_script(wasp_session_t * s, const char *script, xml_t head, size_t len, const unsigned char *data)
{                               // Run script, logs any error, returns malloc'd stdout from script (if any) without newlines
   ...
   char **tmpp = NULL;
   int tmpn = 0;
   int tmp(void) {              // return malloc'd temp file name
   ...
   if (asprintf(env(), "WASPPORT=%s", binding) < 0)
      errx(1, "malloc");
...
static void wasp_message(wasp_session_t * s, size_t len, const unsigned char *data)
{
   if (debug)
...
static void wasp_disconnect(wasp_session_t * s)
{
   if (debug)
...
{                               // Do a command (can be used by connect/disconnect/message) - consumes data
   ...
   {
      if (data)
       websocket_send(t->max, t->ws, len: len, data:data);
                                // Send message before disconnect (consumes data)
      return websocket_send(t->max, t->ws);     // Sends disconnects
   }
   if (!strcmp(cmd, "message"))
    return websocket_send(t->max, t->ws, len: len, data:data);
                                // Send message (consumes data)
...
static const char *wasp_web_command(xml_t head, size_t len, const unsigned char *data)
...
int main(int argc, const char *argv[])
{
   ...
   char *callback_locked(websocket_t * w, xml_t head, size_t datalen, const unsigned char *data) {      // Single threaded callback from web sockets (wrapped in main mutex)
      if (!w && head)
      {                         // Direct run commands
   ...
   char *callback(websocket_t * w, xml_t head, size_t datalen, const unsigned char *data) {     // Callback function - do locking around real callback function
