...
q_thread (void *p)
{
   while (1)
   {
      struct pollfd p = { qpipe[0], POLLIN };
      int s = poll (&p, 1, 60000);
      if (s)
      {                         // Something waiting on pipe
         char poke;
         size_t l = read (qpipe[0], &poke, sizeof (poke));
         if (l < 0)
            err (1, "q pipe failed");
      }
      volatile q_t *q = NULL;
...
main (int argc, const char *argv[])
{
   ...
   {                            // POPT
	 ...
	 // *INDENT-OFF*
         ...
         { "background", 0, POPT_ARG_NONE, &background, 0, "Debug"},
         { "debug", 'v', POPT_ARG_NONE, &debug, 0, "Debug"},
         { "dump", 0, POPT_ARG_NONE, &dump, 0, "WS Dump"},
	 POPT_AUTOHELP { }
	 // *INDENT-ON*
      };
