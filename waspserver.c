...
{				// WASP session
  wasp_session_t **prev, *next;	// Sessions
...
{
  wasp_channel_t **prev, *next;	// Channels
...
{				// Link a channel to a session
  wasp_chanlink_t **cprev, *cnext;	// Chain of channel sessions
  wasp_chanlink_t **sprev, *snext;	// Chain of session channels
...
volatile q_t *queue = NULL, *queueend = NULL;
...
{				// Run command, return static error, consume data
  ...
  if (!strcmp (cmd, "clear") || !strcmp (cmd, "add"))
	...
	{			// Remove all channels first
      ...
      if (data && len)
	{
	  char *p = (char *) data, *e = (char *) data + len;
	  while (p < e)
	    {
	      char *n;
	      for (n = p; n < e && *n >= ' '; n++);
